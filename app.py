...
from modules import vision_target
from modules import vision_target_llava
...
def cached_attack(image_bytes: bytes, attack_category: str, method: str, param):
    ...
    if attack_category == "Noise Attack":
        if method == "Gaussian Noise":
            result = noise_attack.apply_gaussian(image, param)
        else:
            result = noise_attack.apply_pixel_shift(image, param)
    else:
        if method == "EXIF Injection":
            result = stego_attack.inject_exif(image, param)
        else:
            result = stego_attack.inject_lsb(image, param)
...
def cached_defense(image_bytes: bytes, remove_exif: bool, apply_blur: bool, blur_radius: int):
    image = Image.open(io.BytesIO(image_bytes)).convert("RGB")
    result = vision_defense.run_defense_pipeline(
...
def cached_predict(image_bytes: bytes, model_choice: str):
    image = Image.open(io.BytesIO(image_bytes)).convert("RGB")
    if model_choice == "ViT":
        return vision_target.classify_image(image)
    os.makedirs("temp", exist_ok=True)
    temp_path = os.path.join("temp", "temp_image.png")
    image.save(temp_path)
    return vision_target_llava.classify_image(temp_path)
...
if uploaded_file is not None:
    ...
    with tab_results:
        ...
        outcome_cols = st.columns(2 if enable_defense else 1)
